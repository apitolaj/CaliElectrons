...
[ -d "${SRC_BASE_SN}" ] || { echo "ERROR: SRC_BASE does not exist: ${SRC_BASE_SN}"; exit 3; }
...
[ -f "${CONF_SN}" ]           || { echo "ERROR: Config does not exist: ${CONF_SN}"; exit 104; }
...
echo "DEBUG: CONF exists?    $( [ -f "${CONF_SN}" ] && echo yes || echo no )"
